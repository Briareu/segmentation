...
class SegDataset:
    def __init__(self,
        ...
        # original文件夹路径
        self.voc_img_dir = os.path.join(self.data_file, 'data/training_original')
        # mask 文件夹路径
        self.voc_anno_dir = os.path.join(self.data_file, 'data/training_hard')
        # train.txt文件路径
        self.voc_train_lst = os.path.join(self.data_file, 'txts/train1.txt')
        # val.txt文件路径
        self.voc_val_lst = os.path.join(self.data_file, 'txts/val1.txt')
    ...
    def get_mindrecord_dataset(self, is_training, num_shards=1, shuffle=True):
        ...
        for l in lines:
            id_ = l.strip()
            # 修改成jpg？
            img_path = os.path.join(self.voc_img_dir, id_ + '.png')
