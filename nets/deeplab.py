...
def Deeplabv3(input_shape=(512, 512, 3), classes=21, alpha=1.,OS=16):
    ...
    #x = Lambda(lambda xx: tf.image.resize(x, skip1.shape[1:3]))(x) # resize_images
    x = Lambda(lambda x: tf.image.resize(x, skip1.shape[1:3]))(x) # resize_images
    ...
    x = Lambda(lambda xx:tf.image.resize(xx,size_before3[1:3]))(x) # resize_images
